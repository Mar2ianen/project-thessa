...
specific synthesis algorithm. `crates/audio-synth` now provides the first
backend-neutral procedural DSP layer: a live engine source with separate
airborne and structure-borne mixes (flow noise, chamber harmonics,
pump/shaft tone, and low-frequency structural rumble). The Bevy adapter only
wraps that PCM generator as a custom `Decodable`. Remaining `Pitch` tones
are diagnostic one-shots, not the target sound model.
...
1. continuous engine presentation is synthesized procedurally in
   `audio-synth` and gated by resolved airborne versus structure-borne path
   availability;
