...
impl PlanetField {
    ...
    fn height_parts(&self, dir: [f64; 3], min_wavelength_m: f64) -> (f64, f64, f64) {
        ...
        let mountain_detail: f64 = [(8000.0, 1100.0), (4000.0, 450.0)].into_iter()
            .filter(|(wl,_)| *wl >= min_wl)
            .enumerate().map(|(i,(wl,amp))| {
                let p = dir.map(|v|v*self.params.radius_m/wl);
                let n = crate::rng::value_noise3(self.params.seed, 1900+i as u64, p[0],p[1],p[2]);
